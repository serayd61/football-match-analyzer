...
export async function Paywall() {
  const [t, locale] = await Promise.all([getTranslations('paywall'), getLocale()]);
  return (
    <section className="my-8 border border-s-line bg-s-raised/40 px-6 py-12 text-center" aria-labelledby="paywall-title">
      ...
      <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
        <a href={legacyHref(PRICING_HREF, locale)} className={primary}>{t('expiredCta')}</a>
...
export async function TrialNotice({ access }: { access: SiteAccess }) {
  ...
  return (
    <p role="status" className="mb-3 flex flex-wrap items-center justify-between gap-2 border border-s-line bg-s-raised/40 px-3 py-2 text-sm">
      ...
      <a href={legacyHref(PRICING_HREF, locale)} className="underline underline-offset-4">{t('trialCta')}</a>
    </p>
  );
}
  ...
  return (
    <div className="mt-2 border border-dashed border-s-line px-6 py-10 text-center">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-center gap-3">
        <a href={legacyHref(REGISTER_HREF, locale)} className={primary}>{t('lockedCta')}</a>
        <a href={legacyHref(SIGNIN_HREF, locale)} className="text-sm underline underline-offset-4">{t('lockedSignIn')}</a>
      </div>
    </div>
  );
}
