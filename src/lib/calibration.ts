...
let _cache: { curve: CalibrationCurve; at: number } | null = null;
...
/**
 * En güncel kalibrasyon eğrisi. 'covered' segmenti yeterli örnekle fit
 * edildiyse o tercih edilir (modelli ligler kendi rejimini hak eder),
 * yoksa 'all'. Hata durumunda boş eğri → kimlik fonksiyonu.
 */
export async function getCalibration(): Promise<CalibrationCurve> {
  if (_cache && Date.now() - _cache.at < TTL_MS) return _cache.curve;
  const client = sb();
  ...
  try {
    ...
    const covered = data.find((r: any) => r.segment === 'covered');
    const row: any = covered || data.find((r: any) => r.segment === 'all') || data[0];
...
export function invalidateCalibrationCache() { _cache = null; }
