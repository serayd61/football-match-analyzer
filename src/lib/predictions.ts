...
export async function savePrediction(data: PredictionData): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    const devilsAdvocate = reports.devilsAdvocate;
...
export async function updatePredictionResult(
  ...
  try {
    ...
    const updateData = {
      ...
      // Hack: Store Devil's Advocate accuracy in raw_data since we might not have a column
      raw_data: {
        ...prediction.raw_data,
        devils_advocate_match_result_correct: checkMatchResult(prediction.raw_data?.reports?.devilsAdvocate?.matchResult)
      }
    };
