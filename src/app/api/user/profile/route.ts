...
export default function ProfilePage() {
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(lang === 'tr' ? 'Abonelik dönem sonunda iptal edilecek' : 'Subscription will be canceled at end of period');
      } else {
        alert(data.error || 'Error canceling subscription');
      }
    } catch (error) {
  ...
  const labels = {
    tr: {
      ...
      cancelConfirm: 'Aboneliği iptal etmek istediğinizden emin misiniz? Dönem sonuna kadar erişiminiz devam edecek.',
      canceling: 'İptal ediliyor...',
      subscriptionEnds: 'Abonelik bitiş tarihi',
      cancelingStatus: 'İptal Edilecek',
    },
    en: {
      title: 'Profile',
      ...
      cancelConfirm: 'Are you sure you want to cancel? You will have access until the end of your billing period.',
      canceling: 'Canceling...',
      subscriptionEnds: 'Subscription ends',
      cancelingStatus: 'Will be Canceled',
    },
    de: {
      title: 'Profil',
      ...
      cancelConfirm: 'Sind Sie sicher? Sie haben bis zum Ende des Abrechnungszeitraums Zugang.',
      canceling: 'Wird gekündigt...',
      subscriptionEnds: 'Abonnement endet',
      cancelingStatus: 'Wird gekündigt',
    },
  };
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-gray-800/50 backdrop-blur border border-gray-700 rounded-3xl overflow-hidden">
          ...
          <div className="p-6 border-t border-gray-700 space-y-3">
            ...
            {/* Stripe İptal Butonu - Sadece Pro üyeler için */}
            {profile?.isPro && profile?.subscriptionId && (
              <button
                onClick={handleCancelSubscription}
                disabled={canceling}
                className="block w-full py-4 bg-red-500/20 hover:bg-red-500/30 text-red-400 font-medium rounded-xl text-center transition-colors disabled:opacity-50"
              >
                {canceling ? l.canceling : l.cancelSubscription}
              </button>
            )}
