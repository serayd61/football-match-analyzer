...
## Bu projede kullanım (özelleştirme notu)

Bu skill football-match-analyzer için sadeleştirildi: `.nutmeg.user.md` profil dosyası ve `football-docs` MCP sunucusu bu projede YOK. Sağlayıcıya özgü kesin bilgiler (ID şemaları, endpoint'ler, koordinat sistemleri) için varsayım yapma; `nutmeg-learn` glossary'si ve `fa-prediction-lab` skill'indeki proje haritasını kullan. Projenin kendi veri kaynakları: football-data.co.uk (gol+kapanış oranı), Understat/soccerdata (xG), Club Elo, RapidAPI Free API Live Football Data (fikstür/sonuç, FotMob id'leri).
...
## Provider documentation (bu projede)

`football-docs` MCP sunucusu kurulu değil. Sağlayıcıya özgü kesin bilgi gerekirse (Understat/FBref/Opta şemaları, ID köprüleri) WebFetch/WebSearch ile resmi dokümana git ve kaynağı belirt; ezberden ID/qualifier verme. Projede fiilen kullanılan sağlayıcılar ve ad-eşleme katmanları için `fa-prediction-lab/references/project-map.md`'ye bak.
