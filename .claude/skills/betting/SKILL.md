...
## Bu projede kurulum (özelleştirme notu)

`sports-skills` paketi 2026-09-05'te `~/.venvs/sports-skills` venv'ine kuruldu (Python 3.13). CLI yolu:

```bash
$HOME/.venvs/sports-skills/bin/sports-skills <modül> <komut> --param=değer
```

`which sports-skills` boş dönerse bu tam yolu kullan; sistem Python'una `pip install` yapma (Homebrew PEP 668 engeli). Bu projenin canlı verisi RapidAPI (FotMob id'leri) üzerinden gelir; buradaki ESPN/Understat/ClubElo verisi yalnız araştırma ve maç-önü istihbarat içindir, `engine_predictions`'a yazılmaz. Takım id'leri farklı evrenlerdir, karıştırma.
